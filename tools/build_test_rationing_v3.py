...
Eight sequential scenarios driven by a linear script counter
...
Stages:
  ...
  7. priority bands: P1<=2000 must beat P2<=4000
  8. simultaneous: a 2001~4000 receiver also contributes 60
...
# ------------------------------------------------------------------ header
w("//-----------------------------------------------------------------//")
w("//  [TEST MODE] Wartime rationing v3 -- 8 sequential scenarios")
w("//  Script counter: P8 \"Zerg Ultralisk Cavern\" (test-only DC)")
w("//  Leaderboard flip: P8 \"Zerg Defiler Mound\" (test-only DC)")
w("//  Switch246: init guard. Stage bases 0/60/120/180/240/300/360/420, end 480.")
w("//")
...
w("//  7: priority bands P1=1500 must receive before P2=3000")
w("//  8: simultaneous   P1=3000 receives while its own 60 is collected")
...
stage_title(300, "<03>[TEST 6/8] <04>수령자 전무: 전원 8000 보유, 약 4초 후 무산 (자원 변동 없어야 정상)")
stage_waits(300)
...
# ------------------------------------------ stage 7: <=2000 priority
stage_setup(360, "스테이지7 셋업: 2000 이하가 4000 이하보다 우선",
            regs=(1, 2, 3),
            resources=[(1, 1500, 0), (2, 3000, 0), (3, 10000, 0), (4, 0, 0)])
stage_title(360, "<03>[TEST 7/8] <04>구간 우선: P1 1500이 P2 3000보다 먼저 수령해야 합니다.")
stage_waits(360)
check(363, 1, "ore", 2200)
check(364, 2, "ore", 2940)
check(365, 3, "ore", 9360)
stage_advance(360, 420)

# ------------------------- stage 8: tier-1 donor may also be receiver
stage_setup(420, "스테이지8 셋업: 2001~4000 수금과 수령 동시 허용",
            regs=(1, 2),
            resources=[(1, 3000, 0), (2, 10000, 0), (3, 0, 0), (4, 0, 0)])
stage_title(420, "<03>[TEST 8/8] <04>동시 판정: P1은 60을 수금당하면서 P2의 640과 함께 배급받습니다.")
stage_waits(420)
check(423, 1, "ore", 3640)
check(424, 2, "ore", 9360)
stage_advance(420, 480)

trig(HUMANS, "테스트 종료 안내",
     [sw(246, "Set"), dc(SCRIPT, "Exactly", 480)],
     [display("<03>[TEST] <07>배급 테스트 종료 - 8개 시나리오 완료")],
     preserve=False)
