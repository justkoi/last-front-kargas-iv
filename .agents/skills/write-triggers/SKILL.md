...
`tools/` 생성기로 `Triggers/` 또는 테스트 트리거를 재생성하기 전에 **현재 대상 트리거가 생성기보다 최신인지 먼저 확인**한다. 파일에 `Generated by ... -- edit the script, not this file`가 있어도 생성기를 무조건 진실 원본으로 간주하지 않는다.

1. `git diff -- <대상 트리거> <생성기>`와 관련 `git log -p`를 확인한다.
2. 현재 트리거의 조건, 액션, 주석, 모든 플레이어 표시 문구를 생성기 템플릿과 비교한다.
3. 현재 트리거에만 있는 의도적인 최신 변경은 먼저 생성기에 역반영한다. 어느 쪽이 최신인지 불명확하면 재생성하지 말고 사용자에게 알린다.
4. 생성 후 다시 diff를 검토해 요청과 무관한 로직·문구가 되돌아가지 않았는지 확인한다.

특히 안내문·대사·목표 문구는 생성기 문자열보다 **작업 시작 시점의 현재 트리거 문자열**을 보존하는 것을 기본값으로 한다. 생성기 실행 자체는 기존 수동 변경을 덮어쓸 권한이 아니다.
...
- **상태별 비-Preserve 안내 분기는 서로를 비활성화하지 않음** — 예를 들어 `발렌+엘리아 보유`, `발렌만 보유`, `엘리아만 보유` 안내를 각각 비-Preserve 트리거로 만들면 처음 발동한 한 분기만 종료되고 나머지는 계속 대기한다. 이후 한 영웅이 사망·복귀해 `Command` 조합이 달라지면 남은 분기가 최초 배속 안내를 뒤늦게 다시 표시할 수 있다. 상호 배타적 안내 전체를 한 번만 보여야 한다면 소유자 DC로 수신자를 정하고, 각 안내 목적별 공유 완료 래치를 발동 시 함께 set한다. (2026-07-10, 엘리아 첫 복귀 때 배속 안내 재표시)
- **Player 9~12는 `Create Unit` 대상이 될 수 없음** — SCMDraft가 `Create Unit("Player 9", ...)`를 TRIG로 컴파일해도 스타크래프트 엔진은 유닛을 생성하지 않는다. P9~P12 소유 유닛이 필요하면 P1~P8 중 정상 플레이어 소유로 생성한 뒤 `Give Units to Player`로 넘긴다. (2026-07-10, 로크 합류 P9 벌처 생성 실패 실게임 확인)
...
- 사용자가 현재 맵의 유닛·건물 수, 배치 위치, 소유자처럼 **맵 편집에 따라 계속 바뀔 수 있는 값**을 물으면 `Triggers/`의 생성 액션, 주석, `docs/` 문서, 과거 집계값만으로 답하지 않는다.
- 이런 값은 문서나 트리거 주석에 현재값으로 새로 기록하지 않는다. 문서의 숫자는 설계 의도나 과거 스냅샷일 수 있으며 실제 `.scx`와 불일치할 수 있다.
- 먼저 현재 배포·편집 대상 `.scx`를 확인하고, 그 맵에서 `staredit\\scenario.chk`를 추출한 뒤 `UNIT` 섹션의 실제 배치 데이터를 소유자·유닛 종류별로 직접 집계한다. 트리거 생성분을 묻는 경우에만 `TRIG`/TrigEdit를 별도로 분석하여 구분해 보고한다.
- 난이도별 추가 생성이나 AI 확장처럼 런타임에 달라지는 수량은 **맵에 선배치된 수량**과 섞지 말고 별도 항목으로 명확히 구분한다.
- 실제 맵을 직접 조사하지 못했으면 추정값을 현재 수량으로 단정하지 말고, 조사하지 못한 이유를 밝힌다.
